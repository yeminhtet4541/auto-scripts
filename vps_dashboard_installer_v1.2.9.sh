...
if [[ $DISTRO_ID =~ (ubuntu|debian) ]]; then
    apt-get update -y
    # fixed systemd-resolve issue (22.04)
    DEBIAN_FRONTEND=noninteractive apt-get install -y curl wget gnupg2 lsb-release iptables ufw unzip jq software-properties-common git openssl socat openvpn easy-rsa nginx systemd
else
...
read_menu() {
    ...
    case $choice in
        1|01|2|02|3|03|4|04|5|05|6|06) install_xray ;;
        7|07) dns_panel ;; 8|08) domain_panel ;; 9|09) ipv6_panel ;;
        10) vps_status ;; 11) netguard_panel ;; 12) vpn_port_info ;;
